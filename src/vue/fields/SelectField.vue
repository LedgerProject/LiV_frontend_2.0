  ...
  >
    <template v-if="label">
      <label
        ...
        :class="{
          'select-field__label--minimized': modelValue || isListOpened ||
            needAllOption
        }"
      >
        {{ label }}
...
export default {
  props: {
    ...
    needAllOption: { type: Boolean, default: false },
  },
  ...
  data: _ => ({
    ...
    isListOpened: false,
    uniqueId: Date.now() + ((Math.random() * 100000).toFixed()),
  }),

  watch: { modelValue (value) { this.customSelectInstance.value = value } },

  mounted () {
    this.customSelectInstance = customSelect(
      `#select-${this.uniqueId}`, CUSTOM_SELECT_CONFIG,
    )[0]
    ...
    if (!this.modelValue && !this.needAllOption) {
  ...
  methods: {
    ...
    openList () { this.isListOpened = true },

    closeList () { this.isListOpened = false },

    preventKeyDown (e) { e.preventDefault() },
...
.select-field__wrp { position: relative; }
...
.select-field__wrp select { display: none; }
...
.select-field__option--focused {
  background-color: $select-field-hover-background;
}

.select-field__option--selected {
  background-color: $select-field-selected-background;
...
.select-field__selected-icon {
  ...
  &:before { transition: transform 0.2s ease-out; }
...
.select-field__selected-icon--active:before { transform: rotate(-180deg); }

.select-field--disabled,
.select-field--readonly { filter: grayscale(100%); }
